...
public class PersonCardHandle extends NodeHandle<Node> {
    ...
    public boolean equals(Deck deck) {
        return getName().equals(deck.getName().fullName)
                && getAddress().equals(deck.getAddress().value)
                && getPhone().equals(deck.getPhone().value)
                && getEmail().equals(deck.getEmail().value)
                && ImmutableMultiset.copyOf(getTags()).equals(ImmutableMultiset.copyOf(deck.getTags().stream()
                        .map(tag -> tag.tagName)
                        .collect(Collectors.toList())));
    }
}
